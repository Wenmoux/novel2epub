  ...
  return new Promise(async (resolve) => {
    try {
      ...
      const response = await axios.get(
        `https://www.po18.tw/books/${bid}/articlescontent/${pid}`,
        { headers }
      );
...
function getdetail(bid) {
  return new Promise(async (resolve) => {
    try {
      ...
      const tags = [];
      $(".book_intro_tags>a").each((index, element) => {
        const tagText = $(element).text();
        tags.push(tagText);
      });

      let desc = "";
      let descs = $(".B_I_content").text();
      let paragraphs = descs.split(/\s{2,}/);
      paragraphs = paragraphs.map((para) => `<p>${para.trim()}</p>`);
