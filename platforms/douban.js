  ...
  return new Promise(async (resolve) => {
    try {
      let { data } = await axios.get(
        `https://read.douban.com/weixin/miniprogram/hot_fiction/${bid}/reader_data?chapter_id=${pid}`,
        { headers }
      );
      content = "";
        ...
        for (let w = 0; w < json.posts[i].contents.length; w++) {
          if (Object.hasOwn(json.posts[i].contents[w], "data")) {
            ...
            if (data.type === "illus") {
              ...
              content +=
                "<p>"+FuckDouBan_i_have_a_pen_aaa_i_have_a_banana(data.data.text) +
                "</p>";
            }
          }
        }
...
async function getCon(detail) {
  ...
  const { data } = await axios.get(
    `https://read.douban.com/weixin/miniprogram/hot_fiction/${detail.bid}/chapters?start=0&limit=1000000&latestFirst=0&all=0`
  );
  await Promise.all(
    data.list.map((li) => {
      const name = li.title;
...
async function douban(bid) {
  ...
  let data = {
    ...
    operationName: "getColumn",
    variables: { worksId: bid },
  };
  ...
  let detail = {
    ...
    tags:data.tags.slice(0, 3).map(tag => tag.name).join('.'),
    content: [],
    pub: "豆瓣阅读",
    bid,
  };
